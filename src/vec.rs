...
macro_rules! vec_impl_pixel_rgb {
    ($Vec:ident) => {
        ...
        use self::image::{ColorType, Luma, LumaA, Pixel, Primitive};
...
macro_rules! swizzle {
    // READS
    (@read, [$($f:ident),*], $name:ident, ($c1:ident)) => {
        #[inline] pub fn $name(self) -> T { let Self { $($f),* } = self; $c1 }
    };
    (@read, [$($f:ident),*], $name:ident, ($c1:ident, $c2:ident)) => {
        #[inline] pub fn $name(self) -> Vec2<T> { let Self { $($f),* } = self; Vec2 { x: $c1, y: $c2 } }
    };
    (@read, [$($f:ident),*], $name:ident, ($c1:ident, $c2:ident, $c3:ident)) => {
        #[inline] pub fn $name(self) -> Vec3<T> { let Self { $($f),* } = self; Vec3 { x: $c1, y: $c2, z: $c3 } }
    };
    (@read, [$($f:ident),*], $name:ident, ($c1:ident, $c2:ident, $c3:ident, $c4:ident)) => {
        #[inline] pub fn $name(self) -> Vec4<T> { let Self { $($f),* } = self; Vec4 { x: $c1, y: $c2, z: $c3, w: $c4 } }
    };

    // WRITES
    (@write, [$($f:ident),*], $name:ident, ($c1:ident)) => {
        #[inline] pub fn $name(mut self, val: T) -> Self { self.$c1 = val; self }
    };
    (@write, [$($f:ident),*], $name:ident, ($c1:ident, $c2:ident)) => {
        #[inline] pub fn $name(mut self, val: Vec2<T>) -> Self { self.$c1 = val.x; self.$c2 = val.y; self }
    };
    (@write, [$($f:ident),*], $name:ident, ($c1:ident, $c2:ident, $c3:ident)) => {
        #[inline] pub fn $name(mut self, val: Vec3<T>) -> Self { self.$c1 = val.x; self.$c2 = val.y; self.$c3 = val.z; self }
    };
    (@write, [$($f:ident),*], $name:ident, ($c1:ident, $c2:ident, $c3:ident, $c4:ident)) => {
        #[inline] pub fn $name(mut self, val: Vec4<T>) -> Self { self.$c1 = val.x; self.$c2 = val.y; self.$c3 = val.z; self.$c4 = val.w; self }
    };

    // MAIN MUNCHER
    ($mode:ident, $fields:tt, $name:ident = $($c:ident),+; $($rest:tt)*) => {
        swizzle!(@$mode, $fields, $name, ($($c),+));
        swizzle!($mode, $fields, $($rest)*);
    };
    ($mode:ident, $fields:tt,) => {};
}
